...
class _PasswordScreenState extends State<PasswordScreen> {
  ...
  static const _screenTexts = [
    "Set a Strong Password screen.",
    "Create a strong password to protect your account.",
    "A strong password is like a strong lock on your door.",
    "Tips for a strong password.",
    "Use at least 8 characters.",
    "Mix capital and small letters.",
    "Add numbers like 2024.",
    "Add symbols like at the rate, hash, exclamation.",
    "Do not use your name or phone number.",
    "Good example: Kisan at 2024 exclamation.",
    "Tap Continue when your password is strong.",
  ];
  ...
  void checkPasswordStrength(String password) {
    ...
    if (password.length < 6) {
      setState(() { strength = "Very Weak"; strengthColor = Colors.red; strengthValue = 0.2; strengthEmoji = "❌"; });
      context.read<TtsProvider>().speak("Password strength: Very Weak. Make it longer.");
    } else if (password.length >= 6 && (!hasUpper || !hasNumber || !hasSpecial)) {
      setState(() { strength = "Weak"; strengthColor = Colors.orange; strengthValue = 0.4; strengthEmoji = "⚠️"; });
      context.read<TtsProvider>().speak("Password strength: Weak. Add capital letters, numbers, and symbols.");
    } else if (password.length >= 8 && hasUpper && hasLower && hasNumber && hasSpecial) {
      setState(() { strength = "Strong"; strengthColor = Colors.green; strengthValue = 1.0; strengthEmoji = "✅"; });
      context.read<TtsProvider>().speak("Password strength: Strong. Great job!");
    } else {
      setState(() { strength = "Medium"; strengthColor = Colors.blue; strengthValue = 0.65; strengthEmoji = "👍"; });
      context.read<TtsProvider>().speak("Password strength: Medium. Try adding more symbols or numbers.");
    }
  }
  ...
  void submitPassword() {
    if (strength == "Strong") {
      ...
      context.read<TtsProvider>().speak(
        "Your password is not strong enough yet. "
        "Use at least 8 characters, capital letters, numbers, and symbols. "
        "Example: Kisan at 2024 exclamation."
      );
      showDialog(
        context: context,
        builder: (context) => AlertDialog(
            ...
            if (strength.isNotEmpty) ...[
              Row(
                children: [
                  Text(strengthEmoji, style: const TextStyle(fontSize: 22)),
                  const SizedBox(width: 10),
                  Text(strength, style: TextStyle(color: strengthColor, fontWeight: FontWeight.bold, fontSize: 17)),
                ],
              ),
              const SizedBox(height: 8),
              ClipRRect(
